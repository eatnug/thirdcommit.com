...
export class FileSystemPostRepository implements IPostRepository {
   ...
   */
  private parseFilename(filename: string): { id: string | null; slug: string; isLegacy: boolean } {
    const nameWithoutExt = filename.replace(/\.md$/, '')

    // Check if it's new format (contains hyphen separator)
    const separatorIndex = nameWithoutExt.indexOf('-')
    if (separatorIndex > 0 && separatorIndex === 26) { // ULID is exactly 26 chars
   ...
   */
  private async readPostFromFile(fullPath: string, filename: string): Promise<Post | null> {
    try {
      ...
      const { id: parsedId, slug: parsedSlug, isLegacy } = this.parseFilename(filename)
      ...
      if (isLegacy) {
        console.warn(`Legacy post format detected: ${filename}. Consider migrating to new format.`)
      }
...
`
  ...
  async updatePost(
    ...
    const { data: existingFrontmatter, content: existingContent } = matter.default(fileContents)

    // Merge updates with existing data
    const updatedSlug = data.title ? this.slugify(data.title) : existingFrontmatter.slug
    const updatedContent = data.content !== undefined ? data.content : existingContent
    ...
    const updatedFrontmatter = {
      ...
      ...(data.published_at !== undefined && { published_at: data.published_at?.toISOString() || null }),
    ...
    const newFileContent = matter.default.stringify(updatedContent, updatedFrontmatter)
